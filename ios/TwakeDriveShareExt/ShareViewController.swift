...
// NSExtensionPrincipalClass in Info.plist resolves the ObjC runtime name
// "ShareViewController"; @objc(...) keeps that name un-mangled by the Swift module.
@objc(ShareViewController)
class ShareViewController: UIViewController {
  // App Group the app-side ExpoShareIntentModule reads from
  // (Bundle.main["AppGroupIdentifier"]); must match both entitlements files.
  let hostAppGroupIdentifier = "group.com.linagora.twakedrive"
  // Host-app URL scheme the JS useShareIntent(scheme) listens for (twakedrive://dataUrl=).
  let shareProtocol = "twakedrive"
  // Must equal getShareExtensionKey() = `${scheme}ShareKey` used by clearShareIntent().
  let sharedKey = "twakedriveShareKey"
